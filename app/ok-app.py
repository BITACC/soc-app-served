...
UPLOAD_FOLDER_ROOT = "C:\\tmp\\Uploads"
...
# 3) Create a callback
@du.callback(
    output=Output('df',  'data'), #Output("callback-output", "children"),
    id="dash-uploader",
)
def get_dataframe_from_file(filenames):

    df_current = None

    if len(filenames) > 0:
        df_current = pd.read_excel(filenames[0])
    ...
    #return df #html.Ul([html.Li(filenames)])
