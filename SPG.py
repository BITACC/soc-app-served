...
def setup_multi_gpus():
    ...
    #sg from keras.utils.training_utils import multi_gpu_model
    from tensorflow.python.client import device_lib

    # IMPORTANT: Tells tf to not occupy a specific amount of memory
    #sg from keras.backend.tensorflow_backend import set_session  
...
def resample_df(df, sample_rate, combine_all_dwellings=False):
    ...
    def resample_dwelling(df, sample_rate, dwelling_id):
        ...
        if combine_all_dwellings:
            new_df[utils.target + '_std'] = df[utils.target].resample(sample_rate).std() # gives the std (for example that day)
        return new_df
    ...
    if combine_all_dwellings: # Ignore dwelling_ids
        for c in ['eMeter', 'eMeterReturn', 'eMeterLow', 'eMeterLowReturn', 'gasMeter']:
            if c in df.columns :
                df = df.drop(c, axis=1) # Drop columns because they are meaningless when ignoring dwelling ids
...
def downsample_results(x, y_pred, y_true, magnitude, resolution, model_name, savefig=False):
    ...
    result.to_csv(MODEL_DIR + '\\'+model_name+'_'+resolution+'_predictions.csv')
    ...
    plt.ylabel('Corrosion Rate', fontsize=14)
